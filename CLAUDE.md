...
- Every `cat` and `ls` view also gets a full page in `pages/` (`renderPage.ts`): `cat /about/me.md` → `pages/about/me.html`, `ls /about` → `pages/about/index.html`. `ls /` and `tree` views get none; `pages/index.html` (the home page) pre-renders `ls /posts`. A page is `index.html` with three anchors rewritten — `<title>` (replaced by the whole SEO head), the header's `active="…"`, and the `<main …>` open tag, after which the view is injected under a static prompt line. Keep those anchors when editing `index.html`; generation throws otherwise.
- `renderHead.ts` writes each page's SEO head: title (≤ 60 characters) and description (≤ 160), robots, canonical, Open Graph and Twitter Card tags sharing `public/og-image.png`, and JSON-LD (`WebSite` + `Person`, plus `BlogPosting` on posts). `pageMeta.ts` supplies the text: front matter `title` (else the first heading, else the file name) and `description` (else `excerpt`; unlike `excerpt` it is never shown in the view), a dated file counts as an article, sections have hand-written meta in `SECTION_META`, and topic folders are described by the posts they list.
- `renderSitemap.ts` lists every page in `public/sitemap.xml` (gitignored; `public/robots.txt` points at it), except `pages/posts/index.html`, whose canonical is the home page it duplicates.
