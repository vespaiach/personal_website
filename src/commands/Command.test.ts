...
class RequiredArgCommand extends Command {
  ...
  async execute(arg: string | undefined, context: CommandContext): Promise<CommandResult> {
    return { kind: "cwd", cwd: this.resolvePath(arg ?? "", context.cwd) };
  }
}
...
describe("Command.validate", () => {
  it("rejects a missing required arg", () => {
    expect(new RequiredArgCommand().validate(undefined)).toEqual({ valid: false, error: "Usage: req <arg>" });
  });

  it("rejects a required arg that's just whitespace", () => {
    expect(new RequiredArgCommand().validate("   ")).toEqual({ valid: false, error: "Usage: req <arg>" });
  });

  it("accepts a present required arg", () => {
    expect(new RequiredArgCommand().validate("value")).toEqual({ valid: true });
  });

  it("rejects an arg on a no-arg command", () => {
    expect(new NoArgCommand().validate("value")).toEqual({ valid: false, error: "Usage: noop" });
  });

  it("accepts no arg on a no-arg command", () => {
    expect(new NoArgCommand().validate(undefined)).toEqual({ valid: true });
  });

  it("accepts an optional arg with or without a value", () => {
    expect(new OptionalArgCommand().validate(undefined)).toEqual({ valid: true });
    expect(new OptionalArgCommand().validate("value")).toEqual({ valid: true });
  });
});

describe("Command.resolvePath (via a concrete subclass)", () => {
  it("resolves a relative arg against cwd", async () => {
    const result = await new RequiredArgCommand().execute("../posts", { cwd: "/topics", section: "topics" });
    expect(result).toEqual({ kind: "cwd", cwd: "/posts" });
  });

  it("passes an absolute arg through unchanged", async () => {
    const result = await new RequiredArgCommand().execute("/about", { cwd: "/topics", section: "topics" });
